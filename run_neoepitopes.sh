...
#      echo "    -f|--mapqf [INT] Expected alignment score filter  is used for fusions if MAPQ is less than this value (40)"
      echo "    -v|--overlap [INT] Minimum number of mapped nucleotides on both side of the junction (5)"
...
samtools view ${sample}.nullomers.union.ChimerDB.bam | awk -v "sid=${sample}" -v "minMQf=${MINMQf}" -v "maxClip=${MAXCLIP}" -v "S1=${AS1}" -v "B1=${AB1}" -v "slope=${SLOPE}" -v "ovl=${OVERLAP}" -F "\t" '{if(FNR==NR){if($1!=""||NF==6){split($1,u,";");null_list[$3]=$1;junc5[$3]=$4;junc3[$3]=$6;for(i=1;i<length(u);i++){null_neo[$3"_"u[i]]=$2;null_genes[$3"_"u[i]]=$5}}}else{s=0;sc=0;split($6,u,"[A-Z]",v);for(i=1;i<=length(u);i++){if(v[i]=="S"){s=s+(u[i]);sc++}};if(length($10)<(s*maxClip)){next};trulen=length($10)-s;if($4<(500+ov-trulen) || $4>(499-ov)){next};if($5<minMQf){if($12!~/^AS/){next};expS=slope*(trulen - B1) + S1; split($12,alnS,":");if(expS>alnS[3]){next}};split($3,v,".");if(et!=v[1]){et=v[1];nt=split(null_list[et],nlu,";")};for(i=1;i<length(nlu);i++){if(index($10,nlu[i])>0){null_count[et"_"nlu[i]]++}}}}END{for(k in null_count){split(k,w,"_");print sid"\t"w[2]"\t"null_neo[k]"\t"w[1]"\t"null_genes[k]"\t"null_count[k]"\t"junc5[w[1]]"\t"junc3[w[1]]}}' ${pathDB}ChimerDB-nullomersEdge6.tsv - > ${sample}_fusion_neoepitopes_counts_raw.tsv
wait
...
sort -k1,1 -k4,4 -k6n -t$'\t' ${sample}_fusion_neoepitopes_counts_raw.tsv | awk -F "\t" 'BEGIN{print "sample_id\tChimerKB_id\ttop_nullomer\tneoepitopes\tread_count\tnullomer_count\tannotation\tjunction5\tjunction3"}{if(et!=$4){if(FNR!=1 && rcount>1){print pid"\t"et"\t"null"\t"neo_list"\t"rcount"\t"ncount"\t"ann"\t"junc5"\t"junc3}; pid=$1; neo_list=$3; et=$4; ann=$5; junc5=$7; junc3=$8; ncount=0}; ncount++; rcount=$6; null=$2}END{if(rcount>1){print pid"\t"et"\t"null"\t"neo_list"\t"rcount"\t"ncount"\t"ann"\t"junc5"\t"junc3}}' > ${OUTFILE}_fusions.tsv
wait
