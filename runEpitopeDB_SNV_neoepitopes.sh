...
sort -k1,1 -k5,5 -k3,3 -k4n -F "\t" ${sample}_nullomers_neoepitopes_counts_raw.tsv |awk -F "\t" '{if(neo_list!=$3 || et!=$5){if(FNR!=1 && rcount>1){print pid"\t"null"\t"neo_list"\t"rcount"\t"ncount"\t"et}; pid=$1; neo_list=$3; et=$5; ncount=0}; ncount++;rcount=$4; null=$2}END{if(rcount>1){print pid"\t"null"\t"neo_list"\t"rcount}}' > ${sample}_nullomers_neoepitopes_readCounts.tsv
