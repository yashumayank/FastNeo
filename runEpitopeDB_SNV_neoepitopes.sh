...
samtools view ${sample}.nullomers.union.epitopeDB.bam | awk -v "sid=${sample}" -F "\t" '{if(FNR==1){fn++; split(FILENAME,dbtmp,"/");split(dbtmp[length(dbtmp)],dbName,"_n")};if(fn<=3){if($1!=""){split($1,u,";");split($3,uv,"->");if(null_list[$4]==""){ann[$4]=$5;;null_list[$4]=$1;for(i=1;i<=length(u);i++){null_neo[$4"_"u[i]]=$3;null_db[$4"_"u[i]]=dbName[1];null_wtMHC[$4"_"u[i]]="Epitope:"uv[1]","$6;null_neoMHC[$4"_"u[i]]="Epitope:"uv[2]","$7;null_pop[$4"_"u[i]]=$8;}}else{for(i=1;i<=length(u);i++){if(index(null_list[$4],u[i])==0){null_list[$4]=null_list[$4]";"u[i]};if(null_neo[$4"_"u[i]]==""){null_neo[$4"_"u[i]]=$3}else{if(index(null_neo[$4"_"u[i]],$3)==0){null_neo[$4"_"u[i]]=null_neo[$4"_"u[i]]";"$3}};if(null_db[$4"_"u[i]]==""){null_db[$4"_"u[i]]=dbName[1]}else{if(index(null_db[$4"_"u[i]],dbName[1])==0){null_db[$4"_"u[i]]=null_db[$4"_"u[i]]";"dbName[1]}};if(null_wtMHC[$4"_"u[i]]==""){null_wtMHC[$4"_"u[i]]="Epitope:"uv[1]","$6}else{if(index(null_wtMHC[$4"_"u[i]],uv[1])==0){null_wtMHC[$4"_"u[i]]=null_wtMHC[$4"_"u[i]]",Epitope:"uv[1]","$6}};if(null_neoMHC[$4"_"u[i]]==""){null_neoMHC[$4"_"u[i]]="Epitope:"uv[2]","$7}else{if(index(null_neoMHC[$4"_"u[i]],uv[2])==0){null_neoMHC[$4"_"u[i]]=null_neoMHC[$4"_"u[i]]",Epitope:"uv[2]","$7}};if(null_pop[$4"_"u[i]]==""){null_pop[$4"_"u[i]]=$8}else{if(index(null_pop[$4"_"u[i]],$8)==0){null_pop[$4"_"u[i]]=null_pop[$4"_"u[i]]""$8}};}}}}else{if($5<10){if($12!~/^AS/){next};s=0;sc=0;split($6,u,"[A-Z]",v);for(i=1;i<=length(u);i++){if(v[i]=="S"){s=s+(u[i]);sc++}};if(length($10)<(s*3)){next};trulen=length($10)-s;expS=(218*trulen+75)/115; split($12,alnS,":");if(expS>alnS[3]){next}};split($3,v,".");if(et!=v[1]){et=v[1];nt=split(null_list[et],nlu,";")};for(i=1;i<=length(nlu);i++){if(index($10,nlu[i])>0){null_count[et"_"nlu[i]]++}}}}END{for(k in null_count){split(k,w,"_");print sid"\t"w[2]"\t"null_neo[k]"\t"null_count[k]"\t"w[1]"\t"null_db[k]"\t"ann[w[1]]"\t"null_wtMHC[k]"\t"null_neoMHC[k]"\t"null_pop[k]}}' ${pathDB}IEDB_neoepitopes-nullomers.filtered.tsv ${pathDB}TSNAdb_TCGA_neoepitopes-nullomers.filtered.tsv ${pathDB}TSNAdb_ICGC_neoepitopes-nullomers.filtered.tsv  - > ${sample}_epitopeDB_neoepitopes_counts_raw3.tsv
