...
bowtie2 -x ${pathDB}fusion/ChimerDB_bowtie -p $cores --very-sensitive -1 ${sample}_1.nullomers.union.ChimerDB.fastq -2 ${sample}_2.nullomers.union.ChimerDB.fastq -S ${sample}.nullomers.union.ChimerDB.sam
...
#find coverage for each nullomer correspoding to specific neoepitope or gene fusion junction; read length filter
...
bedtools intersect -C -a ${pathDB}TransposElements/TE_cancerRegions.bed -b ${sample}.nullomers.union2.TE.bam | awk '$7>1' >  ${sample}_TE_neoepitopes_readCounts.tsv
wait
