pathDS="cfRNA-neoepitopes"
pathDB=cfRNA-neoepitopes/nullomer_lists/
cd ${pathDS}
cores=2
...
wait
awk '(FNR==1){f++;}{if(f<=2){if(FNR%4==1){a[$1]=1}}else{if(FNR%4==1){if(a[$1]==1){x=1}else{x=0}};if(x==1)print $0 > FILENAME".nullomers.union"}}' ${sample}_*.nullomers_ChimerDB.fastq ${sample}_1.fastq ${sample}_2.fastq
...
wait
rm test1 test2 test ${sample}.nullomers.union.epitopeDB.sam ${sample}.nullomers.union.TE.sam ${sample}.nullomers.union.ChimerDB.sam

samtools view ${sample}.nullomers.union.epitopeDB.bam| awk -v "sid=${sample}" -F "\t" '{if(FNR==1){fn++; split(FILENAME,dbName,"_n")};if(fn<=3){if($1!="" || NF==5){split($1,u,";");if(null_list[$4]==""){null_list[$4]=$1;for(i=1;i<=length(u);i++){null_neo[$4"_"u[i]]=$3;null_db[$4"_"u[i]]=dbName[1]}}else{for(i=1;i<=length(u);i++){if(index(null_list[$4],u[i])==0){null_list[$4]=null_list[$4]";"u[i]};if(null_neo[$4"_"u[i]]==""){null_neo[$4"_"u[i]]=$3}else{if(index(null_neo[$4"_"u[i]],$3)==0){null_neo[$4"_"u[i]]=null_neo[$4"_"u[i]]";"$3}};if(null_db[$4"_"u[i]]==""){null_db[$4"_"u[i]]=dbName[1]}else{if(index(null_db[$4"_"u[i]],dbName[1])==0){null_db[$4"_"u[i]]=null_db[$4"_"u[i]]";"dbName[1]}}}}}}else{split($3,v,".");if(et!=v[1]){et=v[1];nt=split(null_list[et],nlu,";")};for(i=1;i<=length(nlu);i++){if(index($10,nlu[i])>0){null_count[et"_"nlu[i]]++}}}}END{for(k in null_count){split(k,w,"_");print sid"\t"w[2]"\t"null_neo[k]"\t"null_count[k]"\t"w[1]"\t"neo_db[k]}}' ${pathDB}IEDB_neoepitopes-nullomers.filtered.tsv ${pathDB}TSNAdb_TCGA_neoepitopes-nullomers.filtered.tsv ${pathDB}TSNAdb_ICGC_neoepitopes-nullomers.filtered.tsv  - > ${sample}_epitopeDB_neoepitopes_counts_raw.tsv &
samtools view ${sample}.nullomers.union.ChimerDB.bam| awk -v "sid=${sample}" -F "\t" '{if(FNR==NR){if($1!="" || NF==7){split($1,u,";");null_list[$3]=$1;for(i=1;i<length(u);i++){null_neo[$3"_"u[i]]=$2;null_genes[$3"_"u[i]]=$5}}}else{split($3,v,".");if(et!=v[1]){et=v[1];nt=split(null_list[et],nlu,";")};for(i=1;i<length(nlu);i++){if(index($10,nlu[i])>0){null_count[et"_"nlu[i]]++}}}}END{for(k in null_count){split(k,w,"_");print sid"\t"w[2]"\t"null_neo[k]"\t"w[1]"\t"null_genes[k]"\t"null_count[k]}}' ${pathDB}fusion/ChimerKB_n_Pub_junction_nullomers.tsv - > ${sample}_fusions_neoepitopes_counts_raw.tsv
...
sort -k1,1 -k5,5 -k3,3 -k4n ${sample}_epitopeDB_neoepitopes_counts_raw.tsv|awk 'BEGIN{print "sample_id\tnullomer\tneoepitopes\tENST_id\t#reads\t#nullomers"}{if(neo_list!=$3 || et!=$5){if(FNR!=1 && rcount>1){print pid"\t"null"\t"neo_list"\t"et"\t"rcount"\t"ncount}; pid=$1; neo_list=$3; et=$5; ncount=0}; ncount++;rcount=$4; null=$2}END{if(rcount>1){print pid"\t"null"\t"neo_list"\t"et"\t"rcount"\t"ncount}}' > ${sample}_epitopeDB_neoepitopes_readCounts.tsv
sort -k1,1 -k4,4 -k6n -t$'\t' ${sample}_fusions_neoepitopes_counts_raw.tsv |awk -F "\t" 'BEGIN{print "sample_id\tnullomer\tneoepitopes\tENST_id\t#reads\t#nullomers"}{if(et!=$4){if(FNR!=1 && rcount>1){print pid"\t"null"\t"neo_list"\t"et"\t"ann"\t"rcount"\t"ncount}; pid=$1; neo_list=$3; et=$4; ann=$5; ncount=0}; ncount++;rcount=$6; null=$2}END{if(rcount>1){print pid"\t"null"\t"neo_list"\t"et"\t"ann"\t"rcount"\t"ncount}}' > ${sample}_fusions_neoepitopes_readCounts.tsv
rm ${sample}.nullomers.union2.TE.bam
