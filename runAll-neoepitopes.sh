...
bedtools intersect -C -a ${pathDB}TransposElements/TE_cancerRegions.bed -b ${sample}.nullomers.union2.TE.bam | awk -v "sid=${sample}" 'BEGIN{print "sample_id\tTE_ID\t#reads"}{if($7>1){print sid"\t"$4"\t"$7}}' >  ${sample}_TE_neoepitopes_readCounts.tsv
wait
