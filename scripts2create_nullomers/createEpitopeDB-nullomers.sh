...
awk -F "\t" 'BEGIN{c["A"]="T";c["T"]="A";c["G"]="C";c["C"]="G";}{if(NR==FNR){a[$1]=1;y="";for(j=21;j>=1;j--){y=y c[substr($1,j,1)]};rv[y]=$1;next};if($1~/^>.*/){if(seqid!=""){for(i=1;i<=(length(seq)-20);i++){xtr=substr(seq,i,21);if(xtr in a){a[xtr]++};if(xtr in rv){a[rv[xtr]]++}}};seqid=$1;seq=""}else{seq=seq""$1}}END{for(j=1;j<=(length(seq)-20);j++){xtr=substr(seq,j,21);if(xtr in a){a[xtr]++};if(xtr in rv){a[rv[xtr]]++}};for(k in a){print k"\t"a[k]-1}}' epitopeDB21_nullomers.all.tsv /data/hemberg/shared_resources/genomes/human/GRCh38.p13.107.genome.fa > epitopeDB_nullomers_freq.tsv
...
#---For nullomers with genome-frequency < 200, keep first 3 and last 3 nullomers per DNA confirmation of each neoepitope
awk -F "\t" '{if(NR==FNR){f[$1]=$2}else{split($1,u,";");c=0;nlist="";olist="";for(x=1;x<=length(u);x++){if(f[u[x]]<200){nlist=nlist";"u[x];c++};if(c>6){split(substr(nlist,2),v,";");olist=v[1]";"v[2]";"v[3]";"v[length(v)-2]";"v[length(v)-1]";"v[length(v)]}else{olist=substr(nlist,2)}};print olist"\t"$2"\t"$3"\t"$4"\t"$5}}' epitopeDB_nullomers_freq.tsv TSNAdb_ICGC_neoepitopes-nullomers.tsv > TSNAdb_ICGC_neoepitopes-nullomersEdge6.tsv
awk -F "\t" '{if(NR==FNR){f[$1]=$2}else{split($1,u,";");c=0;nlist="";olist="";for(x=1;x<=length(u);x++){if(f[u[x]]<200){nlist=nlist";"u[x];c++};if(c>6){split(substr(nlist,2),v,";");olist=v[1]";"v[2]";"v[3]";"v[length(v)-2]";"v[length(v)-1]";"v[length(v)]}else{olist=substr(nlist,2)}};print olist"\t"$2"\t"$3"\t"$4"\t"$5}}' epitopeDB_nullomers_freq.tsv TSNAdb_TCGA_neoepitopes-nullomers.tsv > TSNAdb_TCGA_neoepitopes-nullomersEdge6.tsv
